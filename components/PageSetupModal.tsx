...
const PageSetupModal: React.FC<PageSetupModalProps> = ({ isOpen, onClose, onApply, pageSettings, t }) => {
  ...
  return (
    ...
    >
      <div 
        className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-200 dark:border-gray-800 animate-in zoom-in-95 duration-300" 
      ...
      >
        <header className="p-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-white dark:bg-gray-900">
            <div className="flex items-center gap-2">
                ...
                <h3 id="page-setup-title" className="text-sm font-bold text-gray-900 dark:text-gray-100 uppercase tracking-widest">{t('modals.pageSetup.title')}</h3>
            </div>
        ...
        <div className="p-6">
            <div className="border-b border-gray-100 dark:border-gray-800">
                <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                    <button 
                        ...
                        className={`whitespace-nowrap py-4 px-1 border-b-2 text-xs font-bold uppercase tracking-widest transition-all ${
                            activeTab === 'pages' 
                    ...
                    <button 
                        disabled 
                        className="whitespace-nowrap py-4 px-1 border-b-2 text-xs font-bold uppercase tracking-widest border-transparent text-gray-300 dark:text-gray-700 cursor-not-allowed"
                    >
            ...
            <div className="pt-8 space-y-8">
                <div className="bg-gray-50 dark:bg-gray-950 p-5 rounded-2xl border border-gray-100 dark:border-gray-800">
                    <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-4">{t('modals.pageSetup.orientation')}</label>
                    <div className="flex items-center gap-8">
                        <label className="inline-flex items-center group cursor-pointer">
                            ...
                            <span className="ml-3 text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-gray-100 transition-colors">{t('modals.pageSetup.portrait')}</span>
                        </label>
                        <label className="inline-flex items-center group cursor-pointer">
                            <div className="relative flex items-center justify-center">
                            ...
                            <span className="ml-3 text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-gray-100 transition-colors">{t('modals.pageSetup.landscape')}</span>
                        </label>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div>
                        <label htmlFor="paper-size" className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 ml-1">{t('modals.pageSetup.paperSize')}</label>
                        <select 
                            id="paper-size" 
                            value={size} 
                            onChange={e => setSize(e.target.value as PageSize)} 
                            className="w-full px-4 py-3 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-sm font-medium transition-all appearance-none cursor-pointer"
                        >
                            {paperSizes.map(ps => <option key={ps.value} value={ps.value}>{ps.label}</option>)}
                        </select>
                    </div>
                    <div>
                        <label htmlFor="page-color" className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 ml-1">{t('modals.pageSetup.pageColor')}</label>
                        <div className="relative group">
                            <input 
                                ...
                                className="w-full h-[46px] p-1.5 border border-gray-200 dark:border-gray-800 rounded-xl cursor-pointer bg-white dark:bg-gray-900 shadow-sm transition-all" 
                            />
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-4 ml-1">{t('modals.pageSetup.margins')}</label>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {(['top', 'bottom', 'left', 'right'] as const).map(side => (
                             <div key={side}>
                                <label htmlFor={`margin-${side}`} className="block text-[9px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-tighter mb-1.5 ml-1">{t(`modals.pageSetup.${side}`)}</label>
                                <input 
                                    type="number" 
                                    id={`margin-${side}`} 
                                    value={margins[side]} 
                                    ...
                                    step="0.1" 
                                    className="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-800 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" 
                                />
                            </div>
                        ))}
                    </div>
        ...
        <footer className="p-5 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-gray-900">
            <button 
                onClick={() => setSetAsDefault(true)} 
                className="px-4 py-2 text-[10px] font-bold text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-all uppercase tracking-widest active:scale-95"
            >
